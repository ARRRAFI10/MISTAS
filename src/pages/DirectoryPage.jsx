...
import { ALUMNI, NOTABLE, cfg } from "../data/alumni";
...
const BATCHES = [
  "All Batches",
  ...Array.from({ length: 27 }, (_, i) => `${1998 + i}`),
];
...
function HallOfFameCarousel() {
  ...
  const visible = Array.from(
    { length: PER_PAGE },
    (_, i) => NOTABLE[(page * PER_PAGE + i) % NOTABLE.length],
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-mono text-xs tracking-widest uppercase text-amber-600 font-bold">
              Hall of Fame
            </span>
          </div>
          <h2 className="font-display text-3xl md:text-4xl text-forest-950 font-bold">
              ...
              return (
                ...
                >
                  ...
                  {a.image ? (
                    ...
                    <>
                      <div
                        className={`absolute inset-0 ${c.bg} opacity-10 rounded-2xl`}
                      />
                      ...
                      >
                        <div
                          className={`w-24 h-24 rounded-full ${c.bg} flex items-center justify-center font-display text-3xl font-bold text-white shadow-xl`}
                        >
                          {a.initials}
                        </div>
                      </div>
                  ...
                  <div className="absolute top-4 left-4 right-4 flex items-start justify-between z-10">
                    <span
                      className={`${c.bg} text-white px-3 py-1 rounded-full text-[10px] font-mono font-bold tracking-wider uppercase shadow`}
                    >
                      {a.dept} · {a.batch}
                    </span>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
                    <h3 className="font-display text-xl text-white font-bold mb-0.5 leading-tight">
                      {a.name}
                    </h3>
                    <p className="font-sans text-forest-300 text-sm mb-3">
                      {a.role} &middot;{" "}
                      <span className="text-forest-400">{a.company}</span>
                    </p>
                    ...
                    {a.award && (
                      <div className="flex items-start gap-2 bg-amber-500/15 border border-amber-500/30 rounded-2xl px-3 py-2.5 mb-4">
                        ...
                        />
                        <span className="font-sans text-xs text-amber-300 leading-snug">
                          {a.award}
                        </span>
                      </div>
                    )}
                  ...
                  <div
                    className={`absolute bottom-0 left-4 right-4 h-0.5 ${c.bg} rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left z-20`}
                  />
                </motion.div>
              );
            })}
...
export default function DirectoryPage() {
  ...
  const hasActiveFilter =
    activeDept !== "All" || selectedBatch !== "All Batches" || notableOnly;
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative bg-forest-950 pt-24 pb-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          >
            <Link to="/" className="hover:text-forest-300 transition-colors">
              Home
            </Link>
          ...
          >
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-display text-2xl text-white font-bold leading-none">
                    {val}
                  </div>
                  <div className="font-mono text-[10px] text-forest-500 tracking-wider uppercase mt-1">
                    {label}
                  </div>
                </div>
              </div>
          ...
          >
            <Search
              size={22}
              className="absolute left-6 top-1/2 -translate-y-1/2 text-forest-400 pointer-events-none"
            />
            <input
              type="text"
              ...
              className="w-full bg-white/95 border-2 border-white text-zinc-900 placeholder-zinc-400 pl-16 pr-14 py-5 rounded-3xl font-sans text-base shadow-2xl focus:outline-none focus:border-forest-400 focus:ring-2 focus:ring-forest-400/30 transition-all"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mb-6 border border-zinc-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {activeDept !== "All" && (
                  <span
                    className={`px-3 py-0.5 rounded-full text-[10px] font-bold font-mono text-white uppercase tracking-wider ${cfg(activeDept).bg}`}
                  >
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.div
                  animate={{ rotate: showDeptFilter ? 180 : 0 }}
                  ...
                  <div className="px-6 py-5 border-t border-zinc-100 bg-zinc-50/80">
                    ...
                    {activeDept !== "All" && (
                      <p className="mt-3 text-xs font-sans text-zinc-400">
                        Showing alumni from:{" "}
                        <strong className={`${cfg(activeDept).text}`}>
                          {cfg(activeDept).label}
                        </strong>
                      </p>
                    )}
                  </div>
          ...
          <div className="flex flex-col gap-6 mb-10 pb-6 border-b border-zinc-100">
            {/* Search in directory */}
            <div className="flex items-center gap-3 bg-zinc-50 px-4 py-3 rounded-xl border border-zinc-200">
              <Search size={16} className="text-zinc-400" />
              <input
                type="text"
                placeholder="Quick search by name, role, or company..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="flex-1 bg-transparent text-sm font-sans text-zinc-900 placeholder-zinc-400 focus:outline-none"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
            ...
            <p className="text-sm font-sans text-zinc-400">
              ...
              {search && (
                <>
                  {" "}
                  matching{" "}
                  <em className="text-zinc-600 not-italic font-semibold">
                    "{search}"
                  </em>
                </>
              )}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="grid sm:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2 gap-6"
            >
              {filtered.map((a, i) => {
                const c = cfg(a.dept);
                return (
                  <motion.div
                    ...
                    className="group bg-white rounded-2xl border border-zinc-200 hover:border-forest-400 hover:shadow-2xl transition-all duration-300 p-7 flex items-start gap-6"
                  >
                    {/* Left - Larger Circular Avatar */}
                    <div
                      className={`flex-shrink-0 w-24 h-24 rounded-full overflow-hidden shadow-lg flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${
                        a.notable
                          ? "ring-3 ring-amber-400"
                          : "ring-3 ring-forest-100"
                      }`}
                    >
                      {a.image ? (
                        <img
                        ...
                        <div
                          className={`w-full h-full ${c.bg} flex items-center justify-center`}
                        >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
                      ...
                      <div>
                        {/* Name */}
                        <h3 className="font-display text-lg font-bold text-zinc-900 line-clamp-2 group-hover:text-forest-800 transition-colors mb-2">
                          {a.name}
                        </h3>

                        {/* Batch & Notable Badge */}
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <span className="px-3 py-1.5 rounded-full text-[10px] font-mono text-zinc-600 bg-zinc-100 border border-zinc-200 uppercase tracking-wider font-bold">
                            Batch {a.batch}
                          </span>
                          {a.notable && (
                      ...
                      {/* Role */}
                      <p className="text-[12px] font-sans text-zinc-500 mb-2 font-semibold">
                        {a.role}
                      </p>
                      ...
                      <div className="flex items-start gap-2 mb-2.5">
                        ...
                        />
                        <span className="text-[11px] font-sans text-forest-600 font-medium line-clamp-2">
                          {a.company}
                        </span>
                      </div>
                      ...
                      <Link
                        to={`/alumni/${a.id}`}
                        className="mt-2 inline-flex items-center gap-2 text-xs font-bold font-sans text-forest-700 hover:text-forest-900 group/link transition-colors"
                      >
          ...
          {filtered.length === 0 && (
            ...
            >
              ...
              <h3 className="font-display text-xl text-zinc-900 font-bold mb-2">
                No Alumni Found
              </h3>
      ...
      <section className="bg-forest-950 py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="font-display text-4xl md:text-5xl text-white font-bold mb-4">
            ...
            <em className="italic text-forest-400">yet?</em>
          </h2>
          <p className="font-body text-forest-300 text-lg mb-8 leading-relaxed max-w-xl mx-auto">
            Join 12,800+ MISTians. Register your profile and stay connected with
            the global MISTAS community.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <Link
              to="/login"
              className="btn-outline px-8 py-3.5 text-base rounded-2xl"
            >
              Update My Profile
            </Link>
          </div>
